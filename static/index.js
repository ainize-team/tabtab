...
if (text) {
  ...
  document.getElementById(
    'editor'
  ).innerText = `Type your word or sentence here and click Run autocomplete or Tab key\n\
...
if (modelUrl) {
  ...
  fetch(
    ...
    .then((response) => {
      ...
      newOption.innerHTML = `Your Model ${
        response.status === 403 ? '🧊' : '🔥'
      }`;
...
if (cp) {
  Array.from(cp).forEach((cp) => cp.addEventListener('click', copyToClipboard));
  ...
  fetch(post_path, {
    ...
    .then((response) => {
      ...
      if (window.innerWidth >= 425) {
        ...
        // for mobile
        menu.style.top = `${bounds.top + 24}px`;
...
document.onkeydown = function () {
  ...
  // TAB 활성화 && ENTER 혹은 (UP, DOWN을 제외한 나머지 키들)
    ...
    TAB_ON == true &&
    (key == KEY_CODE.ENTER || (key != KEY_CODE.UP && key != KEY_CODE.DOWN))
  ) {
...
function showDescription(e) {
  ...
  switch (e.value) {
    case 'gpt2-large':
      ...
      description_link.style.href =
        'https://ainize.ai/Jeong-Hyun-Su/gpt2-large';
      description_link.innerText = 'https://ainize.ai/Jeong-Hyun-Su/gpt2-large';
      break;

    case 'gpt2-cover-letter':
      description.innerHTML =
        'The GPT2-Cover-Letter model generates cover letter style sentence.';
      description_link.href =
        'https://ainize.ai/Jeong-Hyun-Su/gpt2-cover-letter';
    ...
    case 'gpt2-reddit':
      description.innerHTML =
        'The GPT2-Reddit model generates reddit style sentence.';
    ...
    case 'gpt2-story':
      description.innerHTML =
        'The GPT2-Story model generates story style sentence.';
    ...
    case 'gpt2-trump':
      description.innerHTML =
        'This GPT2-trump model generates Donald trump’s tweets style sentence.';
